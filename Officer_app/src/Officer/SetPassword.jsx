...
const SetPassword = () => {
  ...
  const validateToken = async (tokenParam) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || 'http://10.0.0.65:8080'}/api/auth/validate-invitation`,
        { token: tokenParam }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || 'http://10.0.0.65:8080'}/api/auth/set-password`,
        {
          token: token,
